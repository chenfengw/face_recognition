...
def alignment(src_img, src_pts):
    ...
    src_pts: target points for mouse, noise, and eyes
    """
    ...
    tfm = get_similarity_transform_for_PIL(s, r) # 2x3 matrix

    face_img = src_img.transform(crop_size, Image.AFFINE,
            tfm.reshape(6), resample=Image.BILINEAR)
    # print(f"face_img shape: {np.asarray(face_img).shape}")
...
def cropping(src_img):
    ...
    face_img = src_img.crop((left, top, right, bottom))

    return np.asarray(face_img)[:, :, ::-1]
...
def eval_acc(threshold, diff):
    ...
    diff: list. [[name1,name2,cosdistance,sameflag]]

    if cos similary > threshold, then label as same, else 0
    """
    y_true = []
    y_predict = []
    for d in diff:
...
def find_best_threshold(thresholds, predicts):
    """
    thresholds: np.arange(-1.0, 1.0, 0.005). numbers between -1 and 1

    find threshold that give you the best accuracy.
    """
...
for idx, (train, test) in enumerate(folds):
    ...
    best_thresh = find_best_threshold(thresholds, predicts[train] ) # np.arange(-1.0, 1.0, 0.005)
