...
parser.add_argument('--imageRoot', default='/datasets/cs252-sp21-A00-public/hw2_data/CASIA-WebFace', help='path to input images')
...
for epoch in range(0, opt.nepoch):
    ...
    for i, dataBatch in enumerate(faceLoader ):
        ...
        tb_writer.add_scalar('training accuracy', accuracy, iteration)
        ...
        if iteration % 100 == 0:
            ...
            # print('Epoch %d iteration %d: Loss %.5f Accumulated Loss %.5f' % (epoch, iteration, lossArr[-1], 0 ) )
            # print('Epoch %d iteration %d: Accura %.5f Accumulated Accura %.5f' % (epoch, iteration, accuracyArr[-1], 0 ) )
            # trainingLog.write('Epoch %d iteration %d: Loss %.5f Accumulated Loss %.5f \n' % (epoch, iteration, lossArr[-1], 0 ) )
            # trainingLog.write('Epoch %d iteration %d: Accura %.5f Accumulated Accura %.5f\n' % (epoch, iteration, accuracyArr[-1], 0 ) )
